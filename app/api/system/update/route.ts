...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: getErrorMessage(error) || "Versionsprüfung fehlgeschlagen." }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    // Check admin rights if a session is present
    const session = await getCurrentSession();
    if (session && session.role !== "ADMIN") {
      return NextResponse.json({ error: "Zugriff verweigert. Nur Administratoren dürfen Updates ausführen." }, { status: 403 });
    }
    ...
    try {
      ...
      logs.push(`Auf Branch '${targetBranch}' gewechselt.`);
      if (stdout.trim()) logs.push(stdout.trim());
      ...
      try {
        ...
        logs.push(`Checkout Hinweis: ${getErrorMessage(err)}`);
      }
    }
    ...
    try {
      ...
      logs.push("Datenbank-Schema erfolgreich synchronisiert (db push).");
      try {
        ...
        if (msg.includes("EPERM") || msg.includes("query_engine")) {
          logs.push("Prisma Client: DLL während des App-Laufs geladen (wird beim nächsten App-Start aktualisiert).");
        } else {
    ...
    logs.push(`=== Update erfolgreich abgeschlossen. Aktueller Stand: ${updatedCommit} ===`);
    ...
    return NextResponse.json({ error: getErrorMessage(error) || "Update fehlgeschlagen" }, { status: 500 });
  }
}
